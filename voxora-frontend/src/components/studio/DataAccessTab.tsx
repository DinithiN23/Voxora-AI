...
export default function DataAccessTab() {
  ...
  const toggleArea = (area: string) => {
    const current = draft.allowed_data_areas || [];
    if (current.includes(area)) {
      updateDraft({ allowed_data_areas: current.filter((a) => a !== area) });
    } else {
      updateDraft({ allowed_data_areas: [...current, area] });
    }
  };
      ...
      <div className={styles.formGroup}>
        ...
        <div className={styles.chipsContainer}>
          {ALL_BUSINESS_AREAS.map((area) => {
            const isSelected = (draft.allowed_data_areas || []).includes(area);
            return (
              <button
                key={area}
                type="button"
        ...
        <div className={styles.governanceList}>
          ...
          <div className={styles.governanceItem}>
            <div className={styles.governanceInfo}>
              <h4>🛡️ Strict Customer PII Masking</h4>
